...
&emsp;&emsp;我们遵循Leutenegger等人<sup>[17]</sup>的方法，使用Gauss-Newton算法优化滑动窗口中的总误差（8），这在速度和灵活性之间进行了很好的权衡。

&emsp;&emsp;为了便于表示，我们将 (1) 中定义的算子⊞扩展到所有优化参数——对于 $\mathrm{SE}(3)$ 以外的参数，它表示常规加法。我们将使用$\zeta \in \mathrm{SE}(3)^{n} \times \mathbb{R}^{m}$来表示所有的优化变量，包括相机位姿、仿射亮度参数、反深度值和相机内在函数。与<sup>[17]</sup>中一样，边缘化依赖于 $\zeta$ 中的参数的残差将固定在一个切线空间中，在这个切线空间中，有关该参数的任何未来信息(增量更新)都会累积。我们用 $\zeta_0$ 表示这个切线空间的评估点，并用 $\boldsymbol{x} \in \mathfrak{s e}(3)^{n} \times \mathbb{R}^{m}$ 表示积累的增量更新。因此，当前状态估计由 $\zeta=\boldsymbol{x}_{\text {⊞ }} \zeta_{0}$ 给出。图 6 显示了不同变量之间的关系。

**高斯牛顿优化.** 我们计算高斯牛顿系统为:
$$
\mathbf{H}=\mathbf{J}^{T} \mathbf{W} \mathbf{J} \quad \text { and } \quad \mathbf{b}=-\mathbf{J}^{T} \mathbf{W} \mathbf{r} {\tag {10}}
$$
其中 $\mathbf{W} \in \mathbb{R}^{n \times n}$ 是包含权重的对角矩阵，$\mathbf{r} \in \mathbb{R}^{n}$ 是堆叠残差向量，$\mathbf{J} \in \mathbb{R}^{n \times d}$ 是 $\mathbf{r}$ 的雅克比矩阵。

&emsp;&emsp;请注意，每个点对能量贡献 $\mathcal{N}_{\mathbf{p}}$ = 8 个残差。为符号简单起见，我们将在下面只考虑单个残差 $r_k$，
